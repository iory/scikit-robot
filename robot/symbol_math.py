...
def rodrigues2(axis, cosangle, sinangle):
    skewsymmetric = Matrix(3, 3,
                           [S.Zero, -axis[2], axis[1], axis[2], S.Zero, -axis[0], -axis[1], axis[0], S.Zero])
...
def round_matrix(T, precision=8):
    ...
    Teval = T.evalf()
    axisangle = axis_angle_from_matrix([[Teval[0, 0], Teval[0, 1], Teval[0, 2]], [
                                       Teval[1, 0], Teval[1, 1], Teval[1, 2]], [Teval[2, 0], Teval[2, 1], Teval[2, 2]]])
    ...
    if abs(angle) < 10**(- precision):
        ...
        accurateaxisangle = Matrix(
            3, 1, [convert_real_to_rational(x, precision-3) for x in axisangle])
        ...
        if abs(angle-3*pi/2) < 10**(-precision+2):
            quat = [-S.One/sqrt(2), accurateaxisangle[0]/sqrt(2),
                    accurateaxisangle[1]/sqrt(2), accurateaxisangle[2]/sqrt(2)]
        elif abs(angle-pi) < 10**(-precision+2):
            quat = [S.Zero, accurateaxisangle[0],
                    accurateaxisangle[1], accurateaxisangle[2]]
        elif abs(angle-2*pi/3) < 10**(-precision+2):
            quat = [Rational(1, 2), accurateaxisangle[0]*sqrt(3)/2,
                    accurateaxisangle[1]*sqrt(3)/2, accurateaxisangle[2]*sqrt(3)/2]
        elif abs(angle-pi/2) < 10**(-precision+2):
            quat = [S.One/sqrt(2), accurateaxisangle[0]/sqrt(2),
                    accurateaxisangle[1]/sqrt(2), accurateaxisangle[2]/sqrt(2)]
        elif abs(angle-pi/3) < 10**(-precision+2):
            quat = [sqrt(3)/2, accurateaxisangle[0]/2,
                    accurateaxisangle[1]/2, accurateaxisangle[2]/2]
        elif abs(angle-pi/4) < 10**(-precision+2):
            quat = [sqrt(sqrt(2)+2)/2, sqrt(-sqrt(2)+2)/2*accurateaxisangle[0], sqrt(-sqrt(
                2)+2)/2*accurateaxisangle[1], sqrt(-sqrt(2)+2)/2*accurateaxisangle[2]]
        elif abs(angle-pi/6) < 10**(-precision+2):
            quat = [sqrt(2)/4+sqrt(6)/4, (-sqrt(2)/4+sqrt(6)/4)*accurateaxisangle[0], (-sqrt(
                2)/4+sqrt(6)/4)*accurateaxisangle[1], (-sqrt(2)/4+sqrt(6)/4)*accurateaxisangle[2]]
        else:
