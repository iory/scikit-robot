...
def make_gridsdf_self_distance_fn(fk_data, gridsdf_data, backend,
    ...
    get_collision_link_transforms = build_collision_link_transform_fn(
        fk_data, xp, parameterized=parameterized)

    if parameterized:
        def gridsdf_self_distances_from_angles(angles, fk):
            world_pos, world_rot = get_collision_link_transforms(angles, fk)
            return gridsdf_self_distances(world_pos, world_rot, data, xp)
    else:
        def gridsdf_self_distances_from_angles(angles):
            world_pos, world_rot = get_collision_link_transforms(angles)
            return gridsdf_self_distances(world_pos, world_rot, data, xp)
