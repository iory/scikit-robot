...
def util_forward_kinematics(robot_model, link_list, av, move_target, rot_also, base_also, with_jacobian=True):
...
def plan_trajectory_rrt(self, 
    ...
    av_init = util_get_robot_state(self, joint_list)
    def pred_goal_condition(av):
        position = util_forward_kinematics(self, link_list, av, end_effector, False, False, False)
