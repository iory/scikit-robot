...
from skrobot.planner.trajectory_optimization.fk_utils import build_collision_link_transform_fn  # noqa: E402
...
def _make_panda():
    ...
    Only the seven rotational arm joints are optimized: the FK used by the
    solvers applies a rotation per joint, so the prismatic finger joints would
    be placed wrongly.  The fingers are left out of the collision link list
    too, because they are physically adjacent to the hand but far apart in
    that list, and ``create_self_collision_pairs`` only knows about list
    adjacency, so they would show up as a permanent (bogus) contact.
    """
    robot = skrobot.models.Panda()
    robot.reset_manip_pose()
    link_list = [link for link in robot.link_list
                 if link.joint is not None
                 and link.joint.__class__.__name__ == 'RotationalJoint']
...
class TestCollisionLinkTransforms(unittest.TestCase):

    def test_static_link_is_placed_where_it_actually_is(self):
        """A collision link the chain cannot move must not follow the chain.

        The Panda base carries a collision mesh but sits above the first
        optimized joint, so it has no ancestor in ``link_list``.  It used to
        be pinned to chain link 0 with an identity offset, which placed it
        0.333 m away from where it is.
        """
        robot, link_list, collision_link_list = _make_panda()
        problem = TrajectoryProblem(robot, link_list, n_waypoints=2)
        problem.add_collision_cost(
            collision_link_list, world_obstacles=[])

        base = collision_link_list[0]
        self.assertEqual(base.name, 'panda_link0')
        testing.assert_array_equal(
            problem.collision_link_is_static,
            [True] + [False] * (len(collision_link_list) - 1))

        transforms = build_collision_link_transform_fn(
            prepare_fk_data(problem, np), np)
        lower = np.array([link.joint.min_angle for link in link_list])
        upper = np.array([link.joint.max_angle for link in link_list])
        rng = np.random.RandomState(0)

        for trial in range(4):
            angles = (np.array([link.joint.joint_angle()
                                for link in link_list])
                      if trial == 0 else rng.uniform(lower, upper))
            for link, angle in zip(link_list, angles):
                link.joint.joint_angle(float(angle))
            positions, rotations = transforms(angles)
            for i, link in enumerate(collision_link_list):
                testing.assert_allclose(
                    positions[i], link.worldpos(), atol=1e-8,
                    err_msg='{} position'.format(link.name))
                testing.assert_allclose(
                    rotations[i], link.worldrot(), atol=1e-8,
                    err_msg='{} rotation'.format(link.name))
...
class TestGridSDFSelfCollision(unittest.TestCase):
    ...
    def test_jaxls_resolves_self_collision(self):
        ...
        colliding = np.array(
            [1.116, -1.309, 0.575, -3.0, 0.074, 0.008, -0.842])
        n_waypoints = 3
