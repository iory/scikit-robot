...
class TestDifferentiableAxisConstraints(unittest.TestCase):
    ...
    @requires_jax
    def test_batch_ik_position_mask_x(self):
        """Test batch IK with position_mask='x' (constrain x only)."""
        from skrobot.kinematics.differentiable import create_batch_ik_solver
        ...
        # Create target with different x, y, z
        current_pos = move_target.worldpos()
        current_rot = move_target.worldrot()

        # Target: change x by 0.05m, y by 0.1m, z by 0.1m
        target_pos = current_pos + np.array([0.05, 0.1, 0.1])
        target_positions = np.array([target_pos])
        target_rotations = np.array([current_rot])

        initial_angles = np.array([[link.joint.joint_angle() for link in link_list]])

        # Solve with position_mask='x' (only constrain x position)
        solutions, success_flags, errors = solver(
            target_positions,
            target_rotations,
            initial_angles=initial_angles,
            max_iterations=100,
            position_mask='x',  # Only constrain x position
            rotation_mask=True,
        )
        ...
        # X should be close to target (constrained)
        x_error = abs(achieved_pos[0] - target_pos[0])
        self.assertLess(x_error, 0.01, f"X error too large: {x_error}")
        # Y and Z are free, so they may differ from target

    @requires_jax
    def test_batch_ik_rotation_mask_x(self):
        """Test batch IK with rotation_mask='x' (constrain x-axis direction only)."""
        from skrobot.kinematics.differentiable import create_batch_ik_solver
        ...
        # Create target with rotated orientation
        current_pos = move_target.worldpos()

        # Create a rotated target (30 degrees around z-axis)
        from skrobot.coordinates.math import rotation_matrix_from_rpy
        target_rot = rotation_matrix_from_rpy([0.3, 0.2, 0.5])
        ...
        # Solve with rotation_mask='x' (only constrain x-axis direction)
        # Use higher rot_weight for better rotation convergence
        solutions, success_flags, errors = solver(
            target_positions,
            target_rotations,
            initial_angles=initial_angles,
            ...
            rot_weight=1.0,
            position_mask=True,
            rotation_mask='x',  # Only constrain x-axis direction
        )
        ...
        # X-axis direction should match
        target_x_axis = target_rot[:, 0]
        achieved_x_axis = achieved_rot[:, 0]
        x_axis_error = 1 - abs(np.dot(target_x_axis, achieved_x_axis))

        self.assertLess(x_axis_error, 0.01, f"X-axis direction error too large: {x_axis_error}")

    @requires_jax
    def test_batch_ik_position_mask_xy(self):
        """Test batch IK with position_mask='xy' (constrain x and y)."""
        from skrobot.kinematics.differentiable import create_batch_ik_solver
        ...
        # Target: change x, y, z
        target_pos = current_pos + np.array([0.03, 0.05, 0.1])
        target_positions = np.array([target_pos])
        target_rotations = np.array([current_rot])

        initial_angles = np.array([[link.joint.joint_angle() for link in link_list]])

        # Solve with position_mask='xy' (constrain x and y)
        solutions, success_flags, errors = solver(
            target_positions,
            target_rotations,
            initial_angles=initial_angles,
            max_iterations=100,
            position_mask='xy',  # Constrain x and y position
            rotation_mask=True,
        )
        ...
        # X and Y should be close to target (constrained)
        x_error = abs(achieved_pos[0] - target_pos[0])
        y_error = abs(achieved_pos[1] - target_pos[1])
        self.assertLess(x_error, 0.01, f"X error too large: {x_error}")
        self.assertLess(y_error, 0.01, f"Y error too large: {y_error}")
        # Z is free, so we don't check it

    @requires_jax
    def test_batch_ik_no_rotation_constraint(self):
        """Test batch IK with rotation_mask=False (no rotation constraint)."""
        from skrobot.kinematics.differentiable import create_batch_ik_solver
        ...
        # Target position only (rotation doesn't matter)
        target_pos = current_pos + np.array([0.05, 0.0, 0.0])
        target_positions = np.array([target_pos])
        # Random rotation (should be ignored)
        target_rotations = np.array([np.eye(3)])

        initial_angles = np.array([[link.joint.joint_angle() for link in link_list]])

        # Solve with rotation_mask=False
        solutions, success_flags, errors = solver(
            target_positions,
            target_rotations,
            initial_angles=initial_angles,
        ...
        # Apply solution and check position
        for i, link in enumerate(link_list):
            link.joint.joint_angle(solutions[0, i])
