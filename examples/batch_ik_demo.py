...
def main():
    ...
    print("ADVANCED BATCH IK - WITH MASK CONSTRAINTS")
    print("=" * 55)
    print("Configuration:")
    ...
    if success_count > 0:
        ...
        for i, (solution, success) in enumerate(zip(solutions, success_flags)):
            if success:
                ...
                # Mask specifies which axes to CONSTRAIN, so zero out unconstrained axes
                if position_mask is False:
                    constrained_pos_error = np.array([0, 0, 0])
                elif isinstance(position_mask, str):
                    # Zero out axes that are NOT in the mask (unconstrained)
