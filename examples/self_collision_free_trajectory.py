...
GOAL_ANGLES = np.array(
    [1.509, 1.586, -0.472, -2.569, 0.715, 1.894, -1.617])


def make_panda():
    """Return a Panda plus its joint links and self-collision links.

    Only the seven rotational arm joints are optimized: the forward kinematics
    behind the solvers applies one rotation per joint, so the prismatic finger
    joints would be placed wrongly.

    The fingers are excluded from the collision link list as well. They are
    physically attached to the hand but sit far apart in the list, and
